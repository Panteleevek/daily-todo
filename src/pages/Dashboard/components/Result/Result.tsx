...
//   if (Array.isArray(instances) && instances.length > 0) {
    return (
      <div className="flex flex-row fixed bottom-[100px] justify-between max-h-[32px]  items-center w-[100%] mt-auto px-4 ">
        ...
        <div className="flex items-center font-medium text-gray-500 border-1 bg-white h-[44px] ml-4 px-4 rounded-lg hover:bg-gray-100 cursor-pointer rounded-lg transition-colors" onClick={() => dispatch(setModal('create'))}>
          <div>Добавить</div>
        </div>
      </div>
    );
